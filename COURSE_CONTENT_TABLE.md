...
| Concept | Definition / Significance |
| ---- | ---- |
| **Euclidean Distance** | This is a measure of the **magnitude of the difference** between two vectors.<br>Given two vectors `p` and `q`, it is equal to `sqrt( Σ( (p[i] - q[i])^2 ) )`. |
